...
- Set Up Server
  ...
  4. Load KeyStore
  5. Load Certificate 
  6. [Not so optional] load TrustStore
  7. Create Secure Socket 
     1. Server 
...
<p> <img src="./images/HTTPSExchange.png" width="1000"> </p>
...
PKCS8EncodedKeySpec represents the ASN.1 encoding of a private keys
...
Actually on the code I use another method.
...
"A server certificate is sent from the server to the client at the start of a session and is used by the client to authenticate the server"
  - Of the two, server certificates are more commonly used. In fact, it's integral to every SSL or TLS session. Client certificates are not. They're rarely used.
...
/*
  ...
  List of KeyStore Types supported (I think these are types supported by the SUN provider version 14)
    - JKS / BKS (Android)
    - PKCS12 (.p12)
    - JCEKS
